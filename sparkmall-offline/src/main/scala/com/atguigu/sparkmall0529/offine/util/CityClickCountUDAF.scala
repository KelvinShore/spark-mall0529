package com.atguigu.sparkmall0529.offine.util
...
class CityClickCountUDAF extends UserDefinedAggregateFunction{
 ...
 //输入的数据类型 cityname:string
  ...
  //存储类型 map[cityname,count] 总数totalcount
  ...
  override def initialize(buffer: MutableAggregationBuffer): Unit = {
    buffer(0)=new mutable.HashMap[String,Long]()
    buffer(1)=0L
  }

  //分区执行更新操作 把城市名 计数到buffer中
  ...
  override def merge(buffer1: MutableAggregationBuffer, buffer2: Row): Unit = {
    ...
    cityCountMap1.foldLeft(cityCountMap2){case (cityMap2,(cityName,count)) =>
