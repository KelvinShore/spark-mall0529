...
import com.atguigu.sparkmall0529.common.util.JdbcUtil
import org.apache.spark.broadcast.Broadcast
import org.apache.spark.rdd.RDD
import org.apache.spark.sql.SparkSession

/**
  * Created by kelvin on 2019/5/8.
  */
object PageConvertRateApp {
  def calcPageConvertRate(userActionRDD: RDD[UserVisitAction], sparkSession: SparkSession, taskId: String, conditionJsonObj: JSONObject) = {
    // 0 所有规定页面跳转的次数  除以  规定页面的访问次数
    // 求规定页面跳转的次数
    //和 规定页面的访问次数
    //  先求  规定页面的访问次数
    // 1.1 得到规定的页面
    ...
    // 1.2.2 日志按照pageId进行聚合  先过滤再 => 转换成为key => 再聚合统计
    //ps:countbykey是行动得到scala map .reducebykey 得到是rdd 如果没有后续操作可以直接取map
    val pageVisitCountMap: collection.Map[Long, Long] = userActionRDD.filter { userAction =>
      prePageFlowArr.contains(userAction.page_id.toString)
    }.map { userAction => (userAction.page_id, 1L) }.countByKey()

    //3 再求 规定页面跳转的次数
    //3.1 求规定的页面跳转   利用zip进行错位组合1,2,3,4  zip  2,3,4,5 => (1,2)(2,3)(3,4)(4,5) =>(1-2,2-3....)
    ...
    // 3.2   把日志按sessionId 进行聚合 =>  RDD[sessionId,iterable[UserVisitAction]]
    val sessionActionRDD: RDD[(String, Iterable[UserVisitAction])] = userActionRDD.map{userAction =>(userAction.session_id,userAction)}.groupByKey()

    // 3.3 把iterable 按时间进行排序 求出所有的跳转页面
    val pageJumpRDD: RDD[String] = sessionActionRDD.flatMap { case (sessionId, actionItr) =>
      // 3.3.1 按访问时间排序
      val sortedActionList: List[UserVisitAction] = actionItr.toList.sortWith { (userAction1, userAction2) =>
        userAction1.action_time < userAction2.action_time
      }
      //3.3.2 转换 把访问流水  转换成 //1-2 2-4 4-6
      val pageVisitList: List[Long] = sortedActionList.map { userAction => userAction.page_id }
    ...
    val pageJumpList: List[String] = tuplesPageVisitList.map { case (page1, page2) => page1 + "_" + page2 } //1-2 ,2-4,4-6
    val filteredPageJumpList: List[String] = pageJumpList.filter { pageJump => targetPageJumpArrayBC.value.contains(pageJump) } //把不是统计目标的跳转过滤掉
      filteredPageJumpList //每个session的页面跳转  map => flatmap
    ...
    //7 保存到数据库中
    JdbcUtil.executeBatchUpdate("insert into page_convert_rate value(?,?,?)",result)
  }
