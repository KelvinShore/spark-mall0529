...
import com.atguigu.sparkmall0529.offine.bean.{CategoryCountInfo, CategorySession}
...
object CategorySessionApp {
  def statCategoryTop10Session(categoryTop10List: List[CategoryCountInfo], userVisitActionRDD: RDD[UserVisitAction], sparkSession: SparkSession, taskId: String) = {
      // 1 过滤掉不在前十的日志
    ...
    // 统计点击次数   计算每个品类每个session的点击数 => RDD[cid+sessionId,count] => 每个session的次数
    //分组 => RDD[UserVisitAction] => .map=RDD[cid+session,1L]  => reducebykey(_+_)
    val categorySessionCountRDD: RDD[(String, Long)] = filteredActionRDD.map { userAction =>
      (
    ...
    val sessionTop10RDD: RDD[CategorySession] = cidSessionGroupRDD.flatMap { case (cid, sessionItr) =>
      val top10SessionCountList: List[(String, Long)] = sessionItr.toList.sortWith { (sessionCount1, sessionCount2) =>
        ...
        //        println(s"sessionCount1=${sessionCount2._2}")
      ...
      val sessionTop10List: List[CategorySession] = top10SessionCountList.map { case (sessionId, count) =>
        CategorySession(taskId, cid, sessionId, count)
      }
      sessionTop10List
    }
    sessionTop10RDD
    ...
    val paramArray: Array[Array[Any]] = sessionTop10RDD.collect().map(categorySession =>
      Array(categorySession.taskId, categorySession.categoryId, categorySession.sessionId, categorySession.clickCount)
    )
    // 4 保存到mysql
